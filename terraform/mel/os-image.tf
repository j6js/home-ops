data "oci_objectstorage_namespace" "mel_objstr_ns" {
	compartment_id = data.sops_file.oci.data["compartment_ocid"]
}
resource "oci_objectstorage_bucket" "mel_objstr_images_bkt" {
    compartment_id = data.sops_file.oci.data["compartment_ocid"]
    name = "j6js-k3s-objstr-bkt-images"
    namespace = data.oci_objectstorage_namespace.mel_objstr_ns.namespace
}
resource "oci_objectstorage_object" "talos_object_arm64" {
	namespace = data.oci_objectstorage_namespace.mel_objstr_ns.namespace
	bucket    = oci_objectstorage_bucket.mel_objstr_images_bkt.name
...
resource "oci_core_image" "talos_image_arm64" {
    ...
    image_source_details {
        ...
        bucket_name = oci_objectstorage_bucket.mel_objstr_images_bkt.name
        namespace_name = data.oci_objectstorage_namespace.mel_objstr_ns.namespace
